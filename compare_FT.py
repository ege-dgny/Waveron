...
from utils.plotting import (
    ...
    plot_waveron_final_subband_maps_avg_energy,  # Ensured this is called
...
def main(args):
    ...
    training_condition_string_for_plot = f"Trained on {args.dataset}"
    dataset_folder_name_tag = args.dataset

    if args.eval_wavelets_robustness:
        training_condition_string_for_plot = "Trained on Clean MNIST"
        if args.model_types and args.model_types[0] in ['waveron', 'waveron_concat', 'single_layer_waveron_concat']:
            dataset_folder_name_tag = f"{args.model_types[0]}_wavelet_robustness_eval"
        else:
            dataset_folder_name_tag = f"generic_wavelet_robustness_eval"
    elif args.eval_noise_levels:
        dataset_folder_name_tag = f"mnist_clean_train_eval_multi_noise"
        training_condition_string_for_plot = "Trained on Clean MNIST"
    elif args.eval_bg_rand_thresholds:
        dataset_folder_name_tag = f"mnist_clean_train_eval_multi_bgrand"
        training_condition_string_for_plot = "Trained on Clean MNIST"
    elif args.dataset == 'mnist_noisy':
        dataset_folder_name_tag = f"mnist_train_noisy_std{args.noise_std}"
        training_condition_string_for_plot = f"Trained on Noisy MNIST (std={args.noise_std})"
    elif args.dataset == 'mnist_bg_rand':
        dataset_folder_name_tag = f"mnist_train_bg_rand_thr{args.bg_rand_threshold}"
        training_condition_string_for_plot = f"Trained on MNIST + Random BG (thr={args.bg_rand_threshold})"
    elif args.dataset == 'mnist_noisy_bg_rand':
        dataset_folder_name_tag = f"mnist_train_noisy_std{args.noise_std}_bg_rand_thr{args.bg_rand_threshold}"
        training_condition_string_for_plot = f"Trained on Noisy MNIST (std={args.noise_std}) + Random BG (thr={args.bg_rand_threshold})"
    ...
    wavelet_iteration_list = [args.wavelet]
    if args.eval_wavelets_robustness:
    ...
    for current_model_type_arg in args.model_types:
        current_model_wavelet_list = [args.wavelet]
        if args.eval_wavelets_robustness and current_model_type_arg in ['waveron', 'waveron_concat',
                                                                        'single_layer_waveron_concat']:
            current_model_wavelet_list = wavelet_iteration_list

        for current_wavelet_for_model in current_model_wavelet_list:
            print(
                f"\n\n{'=' * 20} Processing Model: {current_model_type_arg.upper()}, Wavelet: {current_wavelet_for_model} {'=' * 20}")

            actual_training_dataset_name = 'mnist' if (
                        args.eval_noise_levels or args.eval_bg_rand_thresholds or args.eval_wavelets_robustness) else args.dataset
            actual_training_noise_std = 0.0 if (
                ...
                args.noise_std if 'noisy' in actual_training_dataset_name else 0.0)
            actual_training_bg_thresh = 0.0 if (
                        args.eval_noise_levels or args.eval_bg_rand_thresholds or args.eval_wavelets_robustness) else (
                args.bg_rand_threshold if 'bg_rand' in actual_training_dataset_name else 0.0)

            print(
                f"Loading training dataset: {actual_training_dataset_name} (Noise: {actual_training_noise_std}, BG: {actual_training_bg_thresh}) ...")
            train_loader, _, num_classes, input_size_hw, input_channels = get_dataset(
                dataset_name=actual_training_dataset_name, data_root=args.data_root,
            ...
            print("Loading clean MNIST test set for baseline plots & epoch evaluation...")
            _, clean_test_loader_for_plots, _, _, _ = get_dataset(
                dataset_name='mnist', batch_size=args.batch_size, data_root=args.data_root,
                target_img_size=(28, 28), noise_std=0.0, bg_rand_threshold=0.0
            )
            ...
            if current_model_type_arg in ['waveron', 'waveron_concat', 'single_layer_waveron_concat']:
                model_specific_tag_for_dir += f"_{current_wavelet_for_model}_k{args.kernel_size}_ch{args.waveron_channels}"
            ...
            print(f"\n--- Training {model_specific_tag_for_legend} on {actual_training_dataset_name} ---")
            ...
            if clean_test_loader_for_plots and len(clean_test_loader_for_plots) > 0:
                ...
                if features.shape[0] > 0: plot_feature_space_projection(features, true_labels,
                                                                        ...
                                                                        save_dir=model_save_directory,
                                                                        dataset_name_for_plot=f"Clean Test (Trained on {actual_training_dataset_name})")
            ...
            if current_model_type_arg in ['waveron', 'waveron_concat', 'single_layer_waveron_concat']:
                ...
                if clean_test_loader_for_plots and len(clean_test_loader_for_plots) > 0:
                    ...
                    # Call new plot for final Waveron subband spatial energy maps
                    plot_waveron_final_subband_maps_avg_energy(model, clean_test_loader_for_plots, device,
                                                               save_dir=model_save_directory,
                                                               dataset_name_for_plot=f"Clean Test (Trained on {actual_training_dataset_name})")
            elif current_model_type_arg == 'fourier':
                if clean_test_loader_for_plots and len(clean_test_loader_for_plots) > 0:
                    # plot_fourier_network_analysis also shows the "pre-classifier" 2D spectrum
    ...
    if args.eval_noise_levels and all_models_noise_evaluation_accuracies:
        print("\n--- Generating Combined Accuracy vs. Noise Plot for all processed model/wavelet variants ---")
        plot_accuracy_vs_noise(
            ...
            training_condition_on_plot=training_condition_string_for_plot
        )

    if args.eval_bg_rand_thresholds and all_models_bg_rand_evaluation_accuracies:
        print(
            "\n--- Generating Combined Accuracy vs. Background Threshold Plot for all processed model/wavelet variants ---")
        plot_accuracy_vs_bg_threshold(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_types', type=str, nargs='+', default=['waveron', 'fourier'],
    ...
    parser.add_argument('--eval_noise_levels', type=float, nargs='*', default=None,
                        help='List of noise std deviations for robustness evaluation. If set, training is on clean MNIST unless overridden by --dataset for non-eval_wavelet runs.')
    parser.add_argument('--eval_bg_rand_thresholds', type=float, nargs='*', default=None,
                        help='List of bg_rand thresholds for robustness evaluation. If set, training is on clean MNIST unless overridden.')
    parser.add_argument('--eval_wavelets_robustness', action='store_true',
                        help='If set, runs robustness evaluation for multiple wavelets (defined in script) on the first specified Waveron model type, training them on clean MNIST and evaluating on noisy MNIST defined by --eval_noise_levels.')
